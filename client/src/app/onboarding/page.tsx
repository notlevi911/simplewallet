...
function generateSeed(words = 12) {
  const list = [
    "ocean","wave","storm","reef","coral","island","current","spray","tidal","whale","shell","pearl",
    "drift","sprout","amber","harbor","siren","azure","mist","delta","brine","gale","lunar","sable",
  ]
  const out: string[] = []
            ...
            >
              ...
              <div className="mt-6 space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <Card>
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-lg bg-white/90 text-[#0a0b0e] flex items-center justify-center"><KeyRound className="w-5 h-5" /></div>
                        <div className="flex-1">
                          <div className="text-white font-semibold">Create New Wallet</div>
                          <div className="text-white/70 text-sm">Generates private key + stealth address. Auto-creates local encrypted store.</div>
                        </div>
                      </div>
                      <div className="mt-4 flex items-center justify-between">
                        <Button onClick={() => { setMode("create"); next() }} className="rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 px-5">Continue</Button>
                        <span className="text-xs text-white/60">Default</span>
                      </div>
                    </Card>
                    <Card>
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-lg bg-white/90 text-[#0a0b0e] flex items-center justify-center"><Wallet className="w-5 h-5" /></div>
                        <div className="flex-1">
                          <div className="text-white font-semibold">Import Existing Wallet</div>
                          <div className="text-white/70 text-sm">Enter seed phrase / private key. Option to import from MetaMask/WalletConnect.</div>
                        </div>
                      </div>
                      <div className="mt-4 space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button onClick={() => { if (onImportValidate()) { setMode("import"); next() } }} className="rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 px-5">Continue</Button>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button className="p-2 rounded-lg border border-white/10 bg-white/10 text-white/80"><Info className="w-5 h-5" /></button>
                            </TooltipTrigger>
                            <TooltipContent side="bottom" className="border-white/15 text-white/85">MetaMask/WalletConnect import coming soon</TooltipContent>
                          </Tooltip>
                        </div>
                      </div>
                    </Card>
                      ...
                      {!seedShown ? (
                        ...
                        <div className="space-y-3">
                          <div className="grid grid-cols-3 gap-2 text-sm">
                            {seed.map((w, i) => (
                              <div key={i} className="px-3 py-2 rounded-lg bg-white/10 border border-white/15 text-white/90 flex items-center gap-2">
                                <span className="text-white/60 text-[11px] w-4">{i + 1}.</span> {w}
                              </div>
                            ))}
                          </div>
                          <div className="text-xs text-white/60">Optional: Encrypt and save seed phrase in your cloud/drive.</div>
                        </div>
                      )}
                    </Card>
                    <Card>
                      ...
                      <div className="mt-4">
                        <Button disabled={!confirmOk} onClick={next} className="rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 px-5 disabled:opacity-50">Continue</Button>
                      </div>
                    </Card>
                  </div>
                )}
                ...
                {step === 2 && mode === "import" && (
                  <Card>
                    <div className="flex items-center gap-3 text-white/85 text-sm">
                      <CheckCircle2 className="w-4 h-4 text-emerald-300" /> Import ready. We created your local encrypted store.
                    </div>
                    <div className="mt-4">
                      <Button onClick={next} className="rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 px-5">Continue</Button>
                    </div>
                  </Card>
                )}
                ...
                {step === 3 && (
                  <Card>
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-lg bg-white/90 text-[#0a0b0e] flex items-center justify-center"><EyeOff className="w-5 h-5" /></div>
                      <div className="flex-1">
                        <div className="text-white font-semibold">Stealth Address Setup</div>
                        <div className="text-white/70 text-sm">Your public address is hidden. We use stealth addresses to protect your activity.</div>
                      </div>
                    </div>
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <Button onClick={rotateStealth} className="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 px-4 py-2 text-sm">
                        <RefreshCw className="w-4 h-4" /> Rotate
                      </Button>
                    </div>
                    <div className="mt-4">
                      <label className="flex items-center gap-2 text-xs text-white/70">
                        <input type="checkbox" checked={advancedStealth} onChange={(e) => setAdvancedStealth(e.target.checked)} />
                      ...
                      {advancedStealth && (
                        <div className="mt-2 flex items-center gap-3">
                          ...
                          <Input
                            ...
                            onChange={(e) => setExtraStealthCount(Math.max(1, Math.min(10, Number(e.target.value) || 1)))}
                    ...
                    <div className="mt-4">
                      <Button onClick={next} className="rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 px-5">Okay, got it</Button>
                    </div>
                  </Card>
                )}
                  ...
                  <Card>
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="text-white/70 text-sm">Choose Retail if you’re just trading privately. Choose Institutional if you need compliance-ready withdrawals.</div>
                      </div>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button className="p-2 rounded-lg border border-white/10 bg-white/10 text-white/80"><Info className="w-5 h-5" /></button>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <button
                        ...
                        className={`px-4 py-4 rounded-xl border transition text-left ${
                          compliance === "retail" ? "bg-white/15 border-white/25 text-white" : "bg-white/10 border-white/15 text-white/85 hover:bg-white/15"
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`px-4 py-4 rounded-xl border transition text-left ${
                          compliance === "institutional" ? "bg-white/15 border-white/25 text-white" : "bg-white/10 border-white/15 text-white/85 hover:bg-white/15"
                        }`}
                      >
                ...
                {step === 6 && (
                  <Card>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-white/70 text-sm mt-1">
                          Mode: {compliance === "retail" ? "Retail (no KYC below threshold)" : "Institutional (zk-attestation enabled)"}
                        </div>
                      </div>
                    </div>
                    ...
                    {confetti && (
                      <div className="mt-4 text-center text-2xl">🎉</div>
                    )}
                  </Card>
                )}
                ...
                {/* Footer nav */}
                <div className="flex items-center justify-between">
                  <Button onClick={prev} disabled={step === 0} variant="outline" className="rounded-full bg-white/10 border border-white/15 text-white/85 hover:bg-white/15 inline-flex items-center gap-2 disabled:opacity-50">
                    <ChevronLeft className="w-4 h-4" /> Back
                  </Button>
                  <div className="text-white/60 text-xs">You can revisit settings later in the app</div>
                  <Button onClick={next} disabled={step === 6} className="rounded-full bg-white/10 border border-white/15 text-white/90 hover:bg-white/15 inline-flex items-center gap-2 disabled:opacity-50">
        ...
        <Dialog open={finishing} onOpenChange={setFinishing}>
          <DialogContent className="border-white/15">
            <DialogHeader>
              ...
              <DialogDescription className="text-white/70">Welcome to Tsunami. Ready to explore your private dashboard.</DialogDescription>
            </DialogHeader>
